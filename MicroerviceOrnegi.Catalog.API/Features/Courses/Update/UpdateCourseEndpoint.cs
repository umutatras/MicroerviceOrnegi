...
namespace MicroerviceOrnegi.Catalog.API.Features.Courses.Update
{
    ...
    public static class UpdateCourseEndpoint
    {
        ...
        {
            group.MapPut("/", async (UpdateCourseCommand Command, IMediator mediator) => (await mediator.Send(Command)).ToGenericResult()).MapToApiVersion(1, 0).AddEndpointFilter<ValidationFilter<UpdateCourseCommand>>().RequireAuthorization("InstructorPolicy");
